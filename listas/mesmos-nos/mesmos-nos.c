...
/* Percorre a árvore e incrementa, em contagem[], a quantidade de vezes
 * que cada caractere aparece como info de algum nó. */
void conta_nos(TNoA *a, int contagem[256]) {
    if (a == NULL)
        return;
    contagem[(unsigned char) a->info]++;
    conta_nos(a->esq, contagem);
    conta_nos(a->dir, contagem);
}
...
int mesmos_nos(TNoA *a1, TNoA *a2) {
    ...
    conta_nos(a1, contagem1);
    conta_nos(a2, contagem2);

    /* as árvores têm os mesmos nós se, para cada valor possível, a
     * quantidade de vezes que ele aparece é igual nas duas árvores
     * (compara como multiconjunto: importa a quantidade, não a posição) */
    for (int i = 0; i < 256; i++) {
