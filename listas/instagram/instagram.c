...
TGrafo *mais_popular(TGrafo *g) {
    ...
    while (v != NULL) {
        int num_seguidores = seguidores(g, v->nome, 0);
        if (num_seguidores > max_seguidores) {
            max_seguidores = num_seguidores;
            popular = v;
        }
        v = v->prox;
    }
...
int segue_mais_velho(TGrafo *g, int imprime) {
    ...
    while (v != NULL) {
        TVizinho *viz = v->prim_vizinho;
        int segue_apenas_velhos = 1;
        int segue_alguem = 0;
        
        while (viz != NULL) {
            segue_alguem = 1;
            TGrafo *v_viz = busca_vertice(g, viz->nome);
            
            if (v_viz != NULL && v_viz->idade <= v->idade) {
                segue_apenas_velhos = 0;
                break;
            }
            viz = viz->prox;
        }
        
        // Verifica se a pessoa segue alguém e se todos que segue são mais velhos
        if (segue_alguem && segue_apenas_velhos) {
            cont++;
            if (imprime) {
                printf("%s ", v->nome);
            }
